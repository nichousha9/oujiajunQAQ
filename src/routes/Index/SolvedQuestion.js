...
@connect(({homens,loading})=>{
  return {
    homens,
    loading:loading.effects['homens/fetchGetMostUsedQuestion'],
  }
})
class SolvedQuestion extends  React.Component{
  componentDidMount(){
  ...
  loadSolvedQuestion =(curTime = []) => {
    ...
    const curDate = curTime.length ? curTime : (dateData.length ? dateData : getTimeForFmortat('today'));
  ...
  renderItem =(item,index) =>{
    return (
      <div span={24} style={{paddingTop:6,paddingBottom:6,width:'100%',display:'flex'}}>
        <div  style={{width:'5%'}}>
          <Icon  className="dot" style={{fontSize:12,marginRight:10,backgroundColor:indexTopColor[index]}} />
        </div>
        <div style={{width:'70%',wordBreak:'break-all'}} >{item.question}</div>
        <div style={{width:'10%'}} className="text-center" >{`${item.rate}%`}</div>
        <div style={{width:'15%'}} className="text-center" >{item.total}</div>
      </div>
    )
  }
  areaData ='' // 当前的地区的数据
  render(){
    const { loading,homens:{mostUsedQuestionList={}} } = this.props; 
    const { DataView } = DataSet;
    const pictureList = mostUsedQuestionList.list ? [...mostUsedQuestionList.list]  : []; 
    if(mostUsedQuestionList.others && !!pictureList.length && pictureList[pictureList.length-1].id !=='others'){
      pictureList.push(mostUsedQuestionList.others)
    }
        ...
        <Spin spinning={loading}>
          <Col span={18}>
            <div className="padding-right-10">
              <CommonList
                height={380}
                dataSource={mostUsedQuestionList.list}
          ...
          <Col span={6}>
            ...
            { !pictureList.length && (
              <div className="text-center margin-top-10">暂无数据</div>
            )}
            { !!pictureList.length && (
              <Chart height={340} data={dv} scale={cols} padding={[ -80, 0, 0, 0 ]} forceFit>
                ...
                <Tooltip
                  showTitle={false}
                  // itemTpl='<li><span style="background-color:{color};" class="g2-tooltip-marker"></span>{name1}: {value1}<p><span style="margin-left:13px" className="g2-tooltip-marker"></span>{name2}: {value1}</p></li>'
                />
                <Geom
