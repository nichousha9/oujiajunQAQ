...
K.options = {
	...
	items : [
		...
		'flash', 'media', 'insertfile', 'capture','table', 'hr', 'emoticons', 'baidumap', 'pagebreak',
...
KEditor.prototype = {
	...
	createMenu : function(options) {
		...
		if(options.name == 'emoticons') options.y -= 250;//表情
	...
	self.afterCreate(function() {
		...
		K(doc.body).bind('paste', function(e){
			    ...
			    if (window.clipboardData) {//ie
					if (clipboardData.files && clipboardData.files.length)//IE11
					    file = clipboardData.files[0];
							...
							alert('暂不支持直接粘贴附件，请使用编辑框工具栏附件上传按钮上传');
					    return true;
					}
			    } else {
			...
			function sendfile(b, t) {
              ...
              fetch(self.uploadJson()+"&paste=true", {
                method: 'POST',
                credentials: 'include',
                body: formData,
              }).then((response)=>{
